...
from tensorflow.keras.layers import Dense, Conv1D, Conv2D, MaxPooling1D, LeakyReLU, Flatten, Input, Dropout, Lambda, Reshape, MaxPooling2D, ReLU, Add
...
print(f"Sample training data (first 5 time steps, channels 0 and 1):\n{X[0, :5, :2]}")
...
mean = np.mean(X, axis=(0, 1), dtype=np.float64)  # shape: (2,)
std = np.std(X, axis=(0, 1), dtype=np.float64)    # shape: (2,)
...
# Broadcasting the (2,) arrays across the sample and time dimensions
X_normalized = (X - mean) / std
X_normalized = X_normalized.astype(np.float64)


print("\nNormalized training data shape:", X_normalized.shape)
print("Sample normalized training data (first 5 time steps, both channels):")
print(X_normalized[0, :5, :])

# Compute and print statistics of the normalized training data to verify normalization
...
# ±5σ boundaries for I channel
I_lower_5sd = I_mean - 5.0 * I_std
I_upper_5sd = I_mean + 5.0 * I_std
...
# Percentile boundaries for Q channel:
...
# ±5σ boundaries for Q channel
Q_lower_5sd = Q_mean - 5.0 * Q_std
Q_upper_5sd = Q_mean + 5.0 * Q_std


print("\nI channel statistics:")
print(f"  Min: {I_min:.5f}, Max: {I_max:.5f}")
print(f"  Mean (float64): {I_mean:.5f}, Median: {I_median:.5f}")
print(f"  Std (float64): {I_std:.5f}")
...
print(f"  5σ range: ({I_lower_5sd:.5f}, {I_upper_5sd:.5f})")

print("\nQ channel statistics:")
print(f"  Min: {Q_min:.5f}, Max: {Q_max:.5f}")
print(f"  Mean (float64): {Q_mean:.5f}, Median: {Q_median:.5f}")
print(f"  Std (float64): {Q_std:.5f}")
...
print(f"  5σ range: ({Q_lower_5sd:.5f}, {Q_upper_5sd:.5f})")


# --- Overall data range using full data (min and max) ---
...
# --- Overall robust boundaries based on channel-specific percentiles ---
# For the 0.1/99.9 pair:
...
print("\nNormalized test sample (first 3 time steps, channels 0 and 1):")
print(X_test_normalized[0, :3, :2])

# Compute the per-channel mean and std for the normalized test data to confirm normalization
test_mean = np.mean(X_test_normalized, axis=(0, 1), dtype=np.float64)
test_std  = np.std(X_test_normalized, axis=(0, 1), dtype=np.float64)

print("\nTest data normalized channel means (should be near 0):", test_mean)
print("Test data normalized channel standard deviations (should be near 1):", test_std)
# =============================================================
...
def representative_data_gen():
    ...
    for batch in dataset:
        clipped = tf.clip_by_value(batch, overall_new_min_0p1, overall_new_max_99p9)
        yield [clipped]
# =============================================================
...
for train_index, val_index in rkf.split(X_normalized):
    ...
    # Apply the custom input quantization layer using the overall new data range
    quantized_inputs = InputQuantizationLayer(min_val=overall_new_min_0p1, max_val=overall_new_max_99p9, name="IQ_quant")(inputs)

    reshaped_inputs = Reshape((1, dim, n_channels))(quantized_inputs)
    ...
    def annotate_layers(layer):
        # If the layer is an Add layer, return it unmodified.
        if isinstance(layer, Add):
            return layer
        # Annotate layers we wish to quantize.
        if isinstance(layer, (Dense, Conv2D, ReLU, Reshape, MaxPooling2D, Flatten, Dropout)):
